    ...
    for k in range(5):
        for n in range(5):
            ...
            for i in range(N):
                xi = (a + h * i)
                yi = yGridA[i]
                yi_plus1 = getYiPlus1ByExplicitMethod(xi, yi, h, k, n)
                step = h / 2
                yi_plus1HalfStep = getYiPlus1ByExplicitMethodHalfStep(xi + h, step, k, n)
                while math.fabs(yi_plus1 - yi_plus1HalfStep) > epsilon:
                    yi_plus1 = yi_plus1HalfStep
                    step = step / 2
                    yi_plus1HalfStep = getYiPlus1ByExplicitMethodHalfStep(xi + h, step, k, n)
                yGridA.append(yi_plus1HalfStep)
                vaults.append(math.fabs(yi_plus1 - yi_plus1HalfStep))
            x = a
            i = 0
            for y in yGridA:
                print("x = {0}; y = {1}; погрешность = {2}".format(x, round(y, 5), round(vaults[i], 6)))
                x = round(x + h, 1)
                i += 1
            ############################################################################################################
