...
if __name__ == "__main__":
    ...
    for k in range(2, N):
        ...
        xGrid.append(xk)
        step = h / 2
        ykHalfStep = getYkByExplicitFormula(xk, step)
        while math.fabs(yk - ykHalfStep) / 3 > epsilon:
            yk = ykHalfStep
            step = step / 2
            ykHalfStep = getYkByExplicitFormula(xk, step)
        yGrid.append(ykHalfStep)
    ...
    for k in range(2, N+1):
        ...
        step = h / 2
        ykHalfStep = getYkByImplicitFormula(xk, step)
        while math.fabs(yk_2 - ykHalfStep) / 7 > epsilon:
            yk_2 = ykHalfStep
            step = step / 2
            ykHalfStep = getYkByImplicitFormula(xk, step)
        yGrid.append(ykHalfStep)
